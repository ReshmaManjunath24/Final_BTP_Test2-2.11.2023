CLASS zcl_annual_notif_reviewer DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_annual_notif_reviewer IMPLEMENTATION.
...
**********************************************************************        lo_mail->add_recipient( iv_address = 'mustafa.alaran@vaspp.com' ).
 ...
 lo_mail->add_recipient( 'sohail.khan@vaspp.com' ).
        ...
        lo_mail->set_subject( iv_subject = 'CLAP-Request: Annaul Review' ).
...
*        iv_message = '<p>Dear Initiator,</p> <p>the The Approver A has added a Query to the following request :</p><p> Reqeust_No: Request1 </p><p> customer: customer1 </p><p> comment: comment1</p> <p>Open Clap Request</p> '.
        iv_message = '<p>Dear Reviewer,</p> <p>Customer is submitted for annual Review :</p><p> Reqeust_No: Request1 </p><p> customer: customer1 '.
*        </p> <p>Open Clap Request</p> '.
        ...
        REPLACE 'Reviewer' WITH ls_reviewer-Name INTO
       iv_message.
    ...
    DATA:lt_result  TYPE TABLE OF ZDD_ANNUAL_NOTIFI_REVIEWER.
...
*
    ...
    <ls_result>-customer = ls_reviewer-customer.
